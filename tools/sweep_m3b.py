#!/usr/bin/env python3
"""Sweep m2 + m3b-step1 over the third-party corpus.

Runs:
- m2 extractor (./build/avif_extract_av1) to /tmp/out.av1
- m3b frame header parser (./build/av1_framehdr)

Goal:
- Robustness: never crash.
- Visibility: how much of the corpus fits the initial m3b subset.
"""
...
def main() -> int:
    ...
    for avif in files:
        ...
        r2 = subprocess.run(
            [str(root / "build" / "av1_framehdr"), "/tmp/out.av1"],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.PIPE,
        )
        if r2.returncode == 0:
    ...
    print("m3b-step1 parse ok:", parse_ok)
    print("m3b-step1 parse fail:", len(parse_fail))

    if parse_fail:
        c = Counter(m for _, m in parse_fail)
        print("\nTop m3b failure reasons:")
